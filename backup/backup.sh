...
LLatest=`date`
Get_Data () {
git clone https://raw.githubusercontent.com/CiaNet-cell/backup.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    DATE=$(date +'%d %B %Y')
    cd /root/user-backup
    git config --global user.email "felicia1110cia@gmail.com" &> /dev/null
    git config --global user.name "CiaNet-cell" &> /dev/null
    rm -rf .git &> /dev/null
    git init &> /dev/null
    git add . &> /dev/null
...
link="https://raw.githubusercontent.com/CiaNet-cell/backup/main/$NameUser/$NameUser.zip"
sleep 1
