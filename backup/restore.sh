...
cd
MYIP=$(curl -sS ipv4.icanhazip.com)
NameUser=$(curl -sS https://raw.githubusercontent.com/CiaNet-cell/ijin/main/ijin | grep $MYIP | awk '{print $2}')
cekdata=$(curl -sS https://github.com/CiaNet-cell/cianet/tree/f53f171f2c42bfc272c50829de9db075503cabbd/backup/$NameUser/$NameUser.zip | grep 404 | awk '{print $1}' | cut -d: -f1)
...
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo -e "$COLOR1┌────────────────────── BY ───────────────────────┐${NC}"
echo -e "$COLOR1│${NC}              • CIANET VPN •                $COLOR1│$NC"
echo -e "$COLOR1└─────────────────────────────────────────────────┘${NC}" 
echo ""
...
wget -q -O /root/backup/backup.zip "https://github.com/CiaNet-cell/cianet/tree/f53f171f2c42bfc272c50829de9db075503cabbd/backup/$NameUser/$NameUser.zip" &> /dev/null
